...
"SELECT Projects.title, (Pledges.amount - Projects.funding_goal) FROM projects INNER JOIN pledges ON projects.id = Pledges.project_id GROUP BY HAVING SUM(pledges.amount) >= projects.funding_goal "
end
